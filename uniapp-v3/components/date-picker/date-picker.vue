...
<script setup lang="ts">
	...
	const DateTools = {
		...
		getHoliday(date) {
			...
			let value = DateTools.format(date, 'mmdd');
	...
	const {isShow, calendarIndex,isMultiSelect ,title,calendars,weeks,isContainTime,showTimePicker,timeValue} = toRefs(state)
	
	//设置值
	const setValue=(value)=> {
		state.date = new Date();
		state.checkeds = [];
		state.isMultiSelect = props.type.indexOf('range') >= 0;
		state.isContainTime = props.type.indexOf('time') >= 0;
		//将字符串解析为Date对象
		...
		if (value) {
			if (state.isMultiSelect) {
				Array.isArray(value) && value.forEach((dateStr, index) => {
					...
					if (index == 0) state.beginTime = time;
					else state.endTime = time;
					state.checkeds.push(date);
				});
			} else {
				if (props.type == 'time') {
					let date = parseDateStr('2019/1/1 ' + value);
					state.beginTime = [date.getHours(), date.getMinutes(), date.getSeconds()];
					onShowTimePicker('begin');
				} else {
					state.checkeds.push(parseDateStr(value));
					if (state.isContainTime) state.beginTime = [
						state.checkeds[0].getHours(),
						state.checkeds[0].getMinutes(),
						state.checkeds[0].getSeconds()
					];
				}
			}
			if (state.checkeds.length) state.date = new Date(state.checkeds[0]);
		} else {
			if (state.isContainTime) {
				state.beginTime = [state.date.getHours(), state.date.getMinutes(), state.date.getSeconds()];
				if (state.isMultiSelect) state.endTime = [...state.beginTime];
			}
			state.checkeds.push(new Date(state.date));
		}
	...
	const onSetYear=(value)=> {
		state.date.setFullYear(state.date.getFullYear() + parseInt(value));
		refreshCalendars(true);
	}
	//改变月份
	const onSetMonth=(value)=> {
		state.date.setMonth(state.date.getMonth() + parseInt(value));
		refreshCalendars(true);
	}
	//时间选择变更
	...
	const onShowTimePicker=(type)=> {
		...
		state.timeType = type;
		state.timeValue = type == 'begin' ? [...state.beginTime] : [...state.endTime];
	}
	//处理日历
	const procCalendar=(item)=> {
		...
		state.checkeds.forEach(date => {
			if (DateTools.isSameDay(date, item.dateObj)) {
				...
				if (state.isMultiSelect && props.showTips) item.tips = props.beginText;
			}
		});
		//节假日或今日的日期标点
		...
		//有两个日期
		if (state.checkeds.length == 2) {
			if (DateTools.isSameDay(state.checkeds[0], item.dateObj)) { //开始日期
				item.bgStyle.type = 'bgbegin';
			}
			if (DateTools.isSameDay(state.checkeds[1], item.dateObj)) { //结束日期
				if (state.isMultiSelect && props.showTips) item.tips = item.bgStyle.type ? props.beginText + ' / ' + props.endText : props.endText;
			...
			if (!item.bgStyle.type && (+item.dateObj > +state.checkeds[0] && +item.dateObj < +state.checkeds[1])) { //中间的日期
	...
	const refreshCalendars=(refresh = false) =>{
		...
		if (state.calendarIndex == 0) {
			if(refresh) state.calendars.splice(0, 1, DateTools.getCalendar(date, procCalendar));
			state.calendars.splice(1, 1, DateTools.getCalendar(after, procCalendar));
			state.calendars.splice(2, 1, DateTools.getCalendar(before, procCalendar));
		} else if (state.calendarIndex == 1) {
			state.calendars.splice(0, 1, DateTools.getCalendar(before, procCalendar));
			if(refresh) state.calendars.splice(1, 1, DateTools.getCalendar(date, procCalendar));
			state.calendars.splice(2, 1, DateTools.getCalendar(after, procCalendar));
		} else if (state.calendarIndex == 2) {
			state.calendars.splice(0, 1, DateTools.getCalendar(after, procCalendar));
			state.calendars.splice(1, 1, DateTools.getCalendar(before, procCalendar));
			if(refresh) state.calendars.splice(2, 1, DateTools.getCalendar(date, procCalendar));
		}
		state.title = DateTools.format(state.date, 'yyyy年mm月');
	}
	//滑块切换
	const onSwiperChange=(e) =>{
		state.calendarIndex = e.detail.current;
		let calendar = state.calendars[state.calendarIndex];
		state.date = new Date(calendar[22].dateObj); //取中间一天，保证是当前的月份
		refreshCalendars();
	}
	//选中日期
	const onSelectDate=(date)=> {
		if (~props.type.indexOf('range') && state.checkeds.length == 2) state.checkeds = [];
		else if (!(~props.type.indexOf('range')) && state.checkeds.length) state.checkeds = [];
		state.checkeds.push(new Date(date.dateObj));
		state.checkeds.sort((a, b) => a - b); //从小到大排序
	...
	//时间选择确定
	const onConfirmTime=()=> {
		if (state.timeType == 'begin') state.beginTime = state.timeValue;
		else state.endTime = state.timeValue;
		state.showTimePicker = false;
	...
	const onConfirm=() =>{
		...
		let fillTime = (date, timeArr) => {
			...
			if (props.showSeconds) date.setSeconds(timeArr[2]);
		};
		if (props.type == 'time') {
			let date = new Date();
			fillTime(date, state.beginTime);
			result.value = DateTools.format(date, props.format ? props.format : defaultFormat.time);
			result.date = date;
		} else {
			if (state.isMultiSelect) {
				let values = [],
					dates = [];
				if (state.checkeds.length < 2) return uni.showToast({
					icon: 'none',
					title: '请选择两个日期'
				});
				state.checkeds.forEach((date, index) => {
					let newDate = new Date(date);
					if (state.isContainTime) {
						let time = [state.beginTime, state.endTime];
						fillTime(newDate, time[index]);
					}
					values.push(DateTools.format(newDate, props.format ? props.format : defaultFormat[state.isContainTime ?
						'datetime' : 'date']));
					dates.push(newDate);
				});
				result.value = values;
				...
				let newDate = new Date(state.checkeds[0]);
				if (state.isContainTime) {
					newDate.setHours(state.beginTime[0], state.beginTime[1]);
					if (props.showSeconds) newDate.setSeconds(state.beginTime[2]);
				}
				result.value = DateTools.format(newDate, props.format ? props.format : defaultFormat[state.isContainTime ?
					'datetime' : 'date']);
				result.date = newDate;
			}
		}
	...
	const computedState=reactive({
		BeginTitle:computed(() => {
			let value = '未选择';
			if (state.checkeds.length) value = DateTools.format(state.checkeds[0], 'yy/mm/dd');
			return value;
		}),
		EndTitle:computed(() => {
			let value = '未选择';
			if (state.checkeds.length == 2) value = DateTools.format(state.checkeds[1], 'yy/mm/dd');
			return value;
		})
	})
	
	const {BeginTitle,EndTitle}= toRefs(computedState)
	
	const PickerTimeTitle = computed(() => {
	    return DateTools.formatTimeArray(state.timeValue, props.showSeconds);
	})
	const BeginTimeTitle = computed(() => {
	    return computedState.BeginTitle != '未选择' ? DateTools.formatTimeArray(state.beginTime, props.showSeconds) : '';
	})
	const EndTimeTitle = computed(() => {
	    return computedState.EndTitle != '未选择' ? DateTools.formatTimeArray(state.endTime, props.showSeconds) : '';
	})

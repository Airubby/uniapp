...
<script setup lang="ts">
	/**
	 * <car-number-input @confirm="numberInputResult" v-model="addInfo.carNumber"></car-number-input>
	 */
	...
	const keyProvince4=ref(['陕', '甘', '青', '宁', '新', '台' ])
	...
	const onPlateDelTap=()=>{
		...
		inputList.value.splice(curInput.value,1," ")
		
		emitResult()
	}
	const emitResult=()=>{
		const returnResult = inputList.value.join("")
		emit('confirm', returnResult);
		emit("update:modelValue", returnResult);
	}
